...
typedef union CmkKeyPath{
    unsigned int *cmk_id;
    char *cmk_name;
} CmkKeyPath;

typedef struct CmkCacheNode {
    CmkKeyStore key_store_type;
    CmkKeyPath cmk_indentify;
    unsigned char *cmk_plain;
...
bool get_cmk_from_cache(CmkCacheList *cmk_cache_list, CmkKeyStore key_store, CmkKeyPath key_path, unsigned char *cmk_plain);
void push_cmk_to_cache(CmkCacheList *cmk_cache_list, CmkKeyStore key_store, CmkKeyPath key_path, const unsigned char *cmk_plian);
