...
bool get_cmk_from_cache(CmkCacheList *cmk_cache_list, CmkKeyStore key_store, unsigned char *cmk_plain)
{
    ...
    if (cur_cmk_node->next == NULL) {
         ...
         */
        if (cur_cmk_node->key_store->cmk_id == key_store->cmk_id && cur_cmk_node->key_store->cmk_name == key_store->cmk_name) {
            for (size_t i = 0; i < DEFAULT_CMK_CACHE_LEN; i++) {
                cmk_plain[i] = cur_cmk_node->cmk_plain[i];
            }
            return true;
